...
var
  ...
  F: TIniFile;
begin
  F := TIniFile.Create(GetPath('data') + 'world.ini');
  try
    S := 'Main';
...
begin
  FMaps[I].Gen;
  FMaps[I].SaveToFile(GetPath('data') + '0.map');
end;
...
begin
  ...
  Lg.Clear;
  Lg.LoadFromFile(GetPath('data') + Lg.Current + '.txt');
  Lg.LoadFromFile(GetPath('data') + Lg.Current + '.world.txt');
  Lg.LoadFromFile(GetPath('data') + Lg.Current + '.tiles.txt');
...
begin
  Player.Clear;
  Saga.ClearLogs;
  World.LoadFromDir(GetPath('data'));
