...
type
  ...
  public
    function ReadCategory(Section, Ident: string; DefaultValue: TCategory): TCategory;
    procedure WriteCategory(Section, Ident: string; Value: TCategory);
    function ReadMaterial(Section, Ident: string; DefaultValue: TMaterial): TMaterial;
...
uses SysUtils, Math, Engine, ForgottenSaga.Game, Common.Map.Tiles, Common.Color, Common.Utils,
  ForgottenSaga.Scenes;

const
  FRaceName: array [TRaceEnum] of string =
    ('Goblin_name1|Goblin_name2|Goblin_name3','Orc_name1|Orc_name2|Orc_name3','Troll_name1|Troll_name2');
    ...
    begin
      ...
      begin
        ...
        FCreature[L].SetPosition(Point(F.ReadInteger(S, 'X', 0),
          F.ReadInteger(S, 'Y', 0)));
        FCreature[I].Atr[atLife].Add(F.ReadString(S, 'Life', Format(BarFmt, [100, 100])));
...
begin
  ...
  try
    ...
    begin
      ...
      F.WriteString(S, 'Life', Format(BarFmt, [FCreature[I].Atr[atLife].Cur, FCreature[I].Atr[atLife].Max]));
...
begin
  ...
  begin
    ...
    if (C = 1) then Saga.World.CurrentItems.Pickup(I)
      else if (C > 1) then Saga.Stages.SetStage(stItems, stGame);
  end;
end;
...
var
  ...
  begin
    ...
    if (I > -1) then
      Result := Format(KeyFmt, [Saga.World.CurrentCreatures.Get(I).Symbol, Saga.World.CurrentCreatures.Get(I).Name]);
  ...
  begin
    ...
    begin
      ...
      C := Saga.World.CurrentItems.Count(Pos.X, Pos.Y);
      if (C > 1) then Result := Format(__('[%s] Несколько (%dx) предметов (%s)'),
        [Saga.World.CurrentItems.Get(I).Symbol, C, Saga.World.CurrentItems.Get(I).Name]);
    end;
  end;
...
begin
  ...
  Saga.Engine.TextOut(0, 39,
    Trim(Format('[%s] %s %s %s', [Saga.Tiles.GetTile(T).Symbol,
    __(Saga.Tiles.GetTile(T).Name), GetCreatures(), GetItems()])));
end;
    ...
    begin
      ...
      begin
        ...
        FItem[L].SetPosition(Point(F.ReadInteger(S, 'X', 0),
          F.ReadInteger(S, 'Y', 0)));
        FItem[L].Calc;
        FItem[L].Durability.Cur := F.ReadInteger(S, 'Durability', FItem[L].Durability.Max);
      end;
    end;
  finally
    F.Free;
...
begin
  Result := '';
  if (Item.Count = 1) then Result := Format('(' + BarFmt + ')', [Item.Durability.Cur, Item.Durability.Max])
    else if (Item.Count > 1) then Result := Format('(%dx)', [Item.Count]);
...
procedure TAdvIniFile.WriteCategory(Section, Ident: string;
  Value: TCategory);
begin
  if (Value = ctNone) then Exit;
  WriteString(Section, Ident, CatStr[Value]);
end;

procedure TAdvIniFile.WriteMaterial(Section, Ident: string;
  Value: TMaterial);
begin
  if (Value = mtNone) then Exit;
